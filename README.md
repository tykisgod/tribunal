...
<p align="center">
  ...
  Claude orchestrates, Codex reviews, every finding is verified against your actual source code.<br><br>
  <i>One judge isn't enough.</i>
</p>
...
| Verification | Reads source to confirm each finding | No independent verification |
...
```
# Backend API project
/tribunal-code --prompt "Check for N+1 queries, missing indexes, and unhandled error codes"

# Game development
/tribunal-plan design.md "Focus on game feel, frame budget, and deterministic simulation"

# Data pipeline
/tribunal-code --prompt "Check for data loss, idempotency, and schema evolution"
```
...
Find the `BASE_BRANCH` variable near the top of `scripts/code-review.sh`:
```bash
BASE_BRANCH="main"
```
...
| 特性 | Tribunal | 典型 AI 审阅 |
|------|----------|-------------|
| 跨模型 | Claude + Codex | 单一模型 |
| 验证 | 读源码确认每条发现 | 无独立验证 |
| 多轮 | 最多 5 轮，自动收敛 | 单轮 |
| 严重度门控 | 无 [Critical] 才停 | 无收敛逻辑 |
| 设计文档 | 支持 | 仅代码 |
...
**改基准分支** — 默认 `develop`，用 `--base main` 或直接改脚本中的 `BASE_BRANCH` 变量。
...
| 特徴 | Tribunal | 一般的なAIレビュー |
|------|----------|------------------|
| クロスモデル | Claude + Codex | 単一モデル |
| 検証 | ソースコードを読んで各指摘を確認 | 独立検証なし |
| マルチラウンド | 最大5ラウンド、自動収束 | シングルパス |
| 重要度ゲーティング | [Critical]がなくなるまでループ | 収束ロジックなし |
| 設計ドキュメント | 対応 | コードのみ |
...
**ベースブランチの変更** — デフォルトは `develop`。`--base main` またはスクリプトの `BASE_BRANCH` 変数を直接編集。
...
- **カスタムプロンプトで的を絞ったレビューを。**「メッセージキューの競合状態をチェック」は汎用的な「レビューして」より効果的。
...
| 특징 | Tribunal | 일반적인 AI 리뷰 |
|------|----------|----------------|
| 크로스 모델 | Claude + Codex | 단일 모델 |
| 검증 | 소스 코드를 읽어 각 발견사항 확인 | 독립 검증 없음 |
| 멀티 라운드 | 최대 5라운드, 자동 수렴 | 단일 패스 |
| 심각도 게이팅 | [Critical] 없을 때까지 루프 | 수렴 로직 없음 |
| 설계 문서 | 지원 | 코드만 |
...
**기준 브랜치 변경** — 기본값은 `develop`. `--base main` 또는 스크립트의 `BASE_BRANCH` 변수를 직접 편집.
...
- **커스텀 프롬프트로 집중 리뷰를.** "메시지 큐의 경쟁 조건 확인"이 범용적인 "리뷰해줘"보다 효과적입니다.
